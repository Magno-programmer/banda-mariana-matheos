...
const Header = () => {
  ...
  const navigationItems = [
    ...
    { name: 'FAQ', path: '/faq', title: 'FAQ - Perguntas Frequentes sobre a Banda' },
    { name: 'Contato', path: '/contato', title: 'Contratar Banda de Jazz - Contato' }
  ];

  const textSize = isMobile ? 'text-lg' : isTablet ? 'text-xl' : 'text-2xl';
  const logoSize = isMobile ? 'w-8 h-8' : isTablet ? 'w-10 h-10' : 'w-12 h-12';

  return (
    <header className="bg-black shadow-sm sticky top-0 z-50 font-gatsbybold" role="banner">

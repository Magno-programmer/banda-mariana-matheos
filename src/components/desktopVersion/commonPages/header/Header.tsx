  ...
  return (
    <header className="bg-black shadow-sm sticky top-0 z-50 font-gatsbybold text-2xl">
      <div className="container mx-auto px-4 py-3">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="md:hidden text-jazz-gold"
            >
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <nav className="md:hidden fixed top-20 right-4 w-56 z-50 bg-black bg-opacity-95 border border-jazz-gold rounded-lg shadow-lg p-4 space-y-4">
            {['Inicio', 'Sobre', 'Repertorio', 'Videos', 'Fotos', 'Agenda', 'Contato'].map((item, i) => (
              <button
                key={i}
                onClick={() => {
                ...
                className="block text-right text-jazz-gold hover:text-white transition-colors border-b border-jazz-gold/20 pb-2 font-gatsby"
              >
                {item}
              </button>
            ))}
